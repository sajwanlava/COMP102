...
public class ClassTimes {
    ...
    public void printCourse(String targetCourse){
        ...
        try {
            ...
            if(course == false){
                UI.println("This course doesn't exist");
            }
            //}
        }
        catch(IOException e) {UI.printf("File Failure %s \n", e);}
    ...
    public void printRoom(String targetRoom) {
        //COULDN'T FIX IRRITATING BUG IN THIS CODE
        ...
        try {
            ...
            if(count==0){
                UI.println("There are " + count + " classes in this room; it doesnt exist");
            }
            else{
                UI.println("There are " + count + " classes in this room");
            }
        }
        catch(IOException e) {UI.printf("File Failure %s \n", e);}
    ...
    public void printInRoomsOnDay(String targetRoom1, String targetRoom2, String targetDay){
        ...
        try{
            Scanner scan = new Scanner(new File("classdata.txt"));
            while(scan.hasNext()){
                String course = scan.next(); //Takes the first thing it finds and assigns it to the vaiable course code
                String classtype = scan.next();
                String day = scan.next();
                String startTime = scan.next();
                String endTime = scan.next();
                String room = scan.next();
                if((room.equals(targetRoom1) || room.equals(targetRoom2)) && day.equals(targetDay)){ //(_ or _) and _)
                    UI.println(course + "  |  " + startTime); //prints course and startTime 
                }
            }
        }
        catch(IOException e){
    ...
    public void bookingsFileForRoom(String targetRoom){
        ...
        try {
            Scanner scan = new Scanner(new File("classdata.txt"));
            PrintStream ps = new PrintStream(new File(targetRoom + "_Bookings.txt"));   //Makes document
            ps.println("Bookings for room " + targetRoom);
            ps.println("----------------------------------");
            while(scan.hasNext()){
                ...
                if(room.equals(targetRoom)){
                    ps.println("Course: " + course);
                    ps.println("Time: " + day + startTime + "-" + endTime);
                    ps.println("Session: " + classType);
                    ps.println("");
                }
            }
        }
