...
public class ImageProcessor{
        ...
        for(int row = 0; row<this.image.length; row++){
            for(int col = 0; col<this.image[row].length; col++){
                if(this.image[row][col] > 128){
                    this.image[row][col] += (this.image[row][col]) * 0.2;
    ...
    public void flipImageHorizontally(){
        /*# YOUR CODE HERE */
        int r = this.image.length; //number of rows
        int c = this.image[0].length; //number of columns 

        int [][] tempArray = new int [r][c];

        for(int row = 0; row<r; row++){
            for(int col = 0; col<c; col++){
                tempArray[row][col] = this.image[row][c - 1 - col];
            }
        }
    ...
    public void flipImageVertically(){
        ...
        for(int row = 0; row<r; row++){
            for(int col = 0; col<c; col++){
                tempArray[row][col] = this.image[r-1-row][col];
            }
        }
    ...
    public void rotateImage180(){
        ...
        for(int row = 0; row<r; row++){
            for(int col = 0; col<c; col++){
                tempArray[row][col] = this.image[r-1-row][c-1-col];
            }
        }
    ...
    public void rotateImage90(){
        ...
        int[][] tempArray= new int[c][r];

        int col = 0;
        for(int i = 0; i < c; i++){
            for(int j = 0; j<r; j++){
                tempArray[i][j] = this.image[r-1-j][col];
            }
            col++;
        }
        this.image = tempArray;
    ...
    public void mergeImage(){
        ...
        for (int row=0; row<rows; row++){
            for (int col=0; col<cols; col++) {
                this.image[row][col] = (other[row][col]+image[row][col])/2;
            }
        }
    }
